...
server.on("request", (request, response) => {
  ...
  response.writeHead(verified ? 200 : 400);
  response.end();
});

suite("HTTP signature", () => {
  suiteSetup(done => server.listen(0, done));

  test("correct key", done => {
    const httpSignature = { keyId: "key-1", key: privateKeyPEMs["key-1"] };
    request(server.url, { httpSignature }, (error, response) => {
  ...
  test("incorrect key", done => {
    const httpSignature = { keyId: "key-2", key: privateKeyPEMs["key-1"] };
    request(server.url, { httpSignature }, (error, response) => {
      assert.deepStrictEqual(error, null);
